...
> 말 움직임, 승리 조건, 첫 판 감각까지 한 번에 잡는 입문용 체스 위키다.

## 체스가 뭐냐면

체스는 8x8 판 위에서 서로 번갈아 말을 움직이며 상대 킹을 몰아붙이는 전략 게임이다.
핵심 목표는 상대 킹을 "잡을 수밖에 없는 상태", 즉 체크메이트로 만드는 것이다.

처음 보면 말 이름도 많고 규칙도 복잡해 보여서 뇌가 살짝 로그아웃할 수 있다.
그래도 순서를 잘 잡으면 "아, 이거 결국 킹 지키면서 공간 싸움하는 게임이네" 하고 감이 온다.
...
1. 백이 먼저 둔다.
2. 각 기물은 움직이는 방식이 다르다.
3. 내 킹이 공격받는 상태가 체크다.
4. 체크를 막을 수 없으면 체크메이트고 게임 종료다.
5. 초반에는 오프닝 이름보다 말이 어떻게 움직이는지와 왜 중앙이 중요한지를 먼저 익히면 된다.

## 처음이면 여기부터 보면 된다

- [chess-basics](docs/guides/chess-basics.md) - 체스가 뭔지, 말이 어떻게 움직이는지, 어떻게 이기는지부터 설명한다.
- [quick-start](docs/guides/quick-start.md) - 판을 실제로 놓고 첫 수를 따라가며 감을 잡는다.
- [prerequisite-map](prerequisite-map.md) - 무엇부터 배우면 덜 꼬이는지 학습 순서를 정리했다.
...
- 체스를 한 번도 안 해봐서 말부터 낯선 사람
- 유튜브를 보기 전에 기본 용어를 먼저 알고 싶은 사람
- 사이트를 이리저리 떠돌지 않고 입문 루트를 한 장에 모아 보고 싶은 사람
