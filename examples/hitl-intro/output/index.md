...
금요일 저녁, 고객에게 대량 메일을 보내는 자동화가 돌고 있다.
모델은 "환불 승인 안내"로 분류했고 발송 직전이다.
그런데 실제 문의 내용은 환불이 아니라 단순 주소 변경 요청이었다.

여기서 사람 확인이 없으면 어떻게 될까.
고객은 엉뚱한 메일을 받고, 운영팀은 월요일에 사고 수습부터 시작한다.
반대로 이 분기가 사람 검토 단계로 한 번만 올라왔으면, 누군가는 "이건 환불이 아니라 주소 변경인데?" 하고 10초 안에 막았을 가능성이 크다.

HITL는 보통 이런 순간에 등장한다.
"AI를 못 믿겠으니 다 사람이 하자"가 아니라, "여긴 틀리면 비싸니까 사람 눈 한 번 거치자"라는 설계다.
...
핵심은 "사람이 다 한다"가 아니라 "기계는 빠르게 돌고, 사람은 사고 나기 쉬운 순간에 개입한다"에 있다.
...
- 자동화는 엔진이다
- HITL는 브레이크이자 핸들이다
- 좋은 설계는 매 단계에 브레이크를 다는 게 아니라, 절벽 앞에만 정확히 다는 것이다
