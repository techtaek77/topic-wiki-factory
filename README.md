...
초보자용 knowledge 위키라면 단순 문서 묶음보다 "허브형 입문 위키"에 가깝게 만들도록 설계돼 있다.
즉 `index.md`가 큰 그림, 5분 요약, 어디부터 읽을지, 자주 찾는 규칙, 외부 학습 자료까지 한 번에 안내하는 홈 역할을 맡는다.
...
특히 `examples/chess-intro/`는 아래 흐름을 예시로 보여 준다.
