...
6. `WRITING-STYLE.md`, `HYBRID-WRITING-STYLE.md`, `DIAGRAM-GUIDELINES.md` → 문체/도해 원칙 참조
...
- 단계가 3개 이상인 흐름, 분기 2개 이상인 판단 구조, 문서/개념 의존성이 3개 이상인 구조는 mermaid 차트를 우선 검토한다.
  - 단계 흐름/상태 전이/승인 절차: `mermaid flowchart`
  - 선수지식/문서 의존성/개념 관계: `mermaid graph`
  - 공간 배치/before-after/이동 경로: SVG 또는 정적 이미지
- 차트를 넣는다면 앞에 1~3문장으로 "이 그림이 무엇을 보여 주는지"를 먼저 설명한다.
- 차트는 장식이 아니라 이해 속도를 높이는 압축이어야 한다.
