...
6. `WRITING-STYLE.md`, `HYBRID-WRITING-STYLE.md` → 문체 원칙 참조
7. {slug}가 `docs_to_revise`에 있으면 → wiki-memory.md의 `수정 이유 로그` 확인
...
- 추상적인 개념, 판단 감각, 운영 감각이 중요한 문서라면 짧은 `장면형 도입`을 사용할 수 있다.
  - `누가 -> 무엇을 하려는가 -> 어디서 헷갈리거나 틀리는가 -> 왜 중요한가`
  - 장면은 4~8문장 안쪽, plausible한 상황으로 제한한다.
  - 장면 뒤에는 반드시 개념 설명이나 배워야 할 원칙이 바로 따라와야 한다.
...
- 유머와 비유는 허용하지만 설명을 돕는 범위에서만 쓴다.
  - 목표는 웃기기보다 덜 겁먹게 하고 더 오래 기억하게 만드는 것이다.
  - 문단마다 드립을 넣지 않는다.
- 추상어를 길게 설명하기보다 실제 상황 예시를 먼저 두고, 원칙은 그 해설처럼 붙인다.
- 소제목은 멋보다 설명을 우선한다.
...
✅ 추상적 개념 문서라면 짧은 장면, 비유, 실무 상황 중 하나로 감을 먼저 잡게 했는가?
✅ 선수 지식이 2개 이하인가?
✅ 실전 예시가 1개 이상인가?
✅ 자주 하는 실수가 2개 이상인가?
...
✅ index라면 큰 그림 전에 짧은 장면이나 공감 질문으로 왜 중요한지 감을 줄 수 있는가?
```
