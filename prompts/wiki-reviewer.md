...
### A. `concept` / `guide` 문서 (11개 항목)

각 문서에 대해 확인:

```
[ ] 1. 한 줄 설명 있음 (전문용어 없이 target_audience 언어)
[ ] 2. 초반 5문장 안에 왜 읽어야 하는지 보임
[ ] 3. 추상적 개념 문서라면 장면/비유/실무 상황 중 하나로 감을 먼저 잡게 함
[ ] 4. 선수 지식 ≤ 2개, [[링크]] 형식
[ ] 5. 실전 예시 1개 이상
[ ] 6. 자주 하는 실수 2개 이상
[ ] 7. "더 깊이 가려면" 링크 있음
[ ] 8. 전문 용어가 glossary와 [[링크]] 연결됨
[ ] 9. 과장 표현 없음 ("완벽한", "최고의", "혁신적인")
[ ] 10. 파일명 kebab-case
...
```
[ ] 한 줄 소개 있음
[ ] 필요할 경우 짧은 장면/공감 질문으로 왜 중요한지 감을 줌
...
```
[ ] 질문을 남기는 목적이 설명됨
[ ] 이미 답이 있는 질문과 보강 후보 질문이 구분됨
[ ] 에이전트에게 다시 물을 때 필요한 정보가 적혀 있음
```
...
- `concept` / `guide`: 12/12이면 PASS, 아니면 REVISE
