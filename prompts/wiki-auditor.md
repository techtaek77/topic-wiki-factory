...
당신은 위키 감사관입니다. `{output_path}/` 아래의 계획 문서 전체를 9가지 항목으로 점검하세요.
...
**9. 질문 흡수 장치 부재** (warning)
knowledge 유형 + 초보자 대상인데 `questions.md`가 없거나, `index.md`/`faq.md`에서 질문 보관함으로 가는 길이 없으면 경고.
